...
export class AppointmentController {
  ...
  static async cancel(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const appointment = await AppointmentService.updateStatus(req.params.id, req.user!, 'CANCELLED');
      res.json(appointment);
    } catch (err) {
      next(err);
    }
  ...
  static async complete(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const appointment = await AppointmentService.updateStatus(req.params.id, req.user!, 'COMPLETED');
      res.json(appointment);
    } catch (err) {
      next(err);
    }
